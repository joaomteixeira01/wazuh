...
inline bool startsWith(const std::string& value, const std::string_view prefix)
{
...
inline std::string parseStagePrefix()
{
    return std::string(syntax::asset::PARSE_KEY) + "|";
}
...
inline std::string allowedStages(const std::string& assetType)
{
...
inline void validateDecoderStage(const std::string& stage, const base::Name& assetName)
{
    ...
    const auto parsePrefix = parseStagePrefix();
    if (!startsWith(stage, parsePrefix))
    {
        return;
    }

    const auto field = stage.substr(parsePrefix.size());
