...
remove_service_units() {
    ...
    if [ -f /etc/rc.local ]; then
        sed -i '/wazuh-control start/d;/wazuh-manager-control start/d' /etc/rc.local 2>/dev/null || true
    fi

    for init_dir in /etc/init.d /etc/rc0.d /etc/rc1.d /etc/rc2.d /etc/rc3.d /etc/rc4.d /etc/rc5.d /etc/rc6.d; do
        [ -d "${init_dir}" ] || continue
        find "${init_dir}" -maxdepth 1 -name '*wazuh*' -exec rm -f {} + 2>/dev/null || true
    done

    if command -v systemctl >/dev/null 2>&1; then
        for service_dir in /etc/systemd/system /usr/lib/systemd/system /lib/systemd/system; do
            [ -d "${service_dir}" ] || continue
            find "${service_dir}" -maxdepth 2 -name 'wazuh*' -exec rm -f {} + 2>/dev/null || true
        done
